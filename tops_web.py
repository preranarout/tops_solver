...
# import library_plot as libplot 
...
def dinkelbach_solve(x_0, weights, obj_func, dink_model, deviation, op_type):
	...
	while (continue_flag == True):
		...
		# if (k > 0):
		# 	plot_obj_values.append(F_k)
		libio.pprint(libio.YELLOW_TEXT, ('Obj val  : ' + str(lph.solveLinearExpression(obj_func, x_k))))
		if (k > 0):
			plot_obj_values.append(lph.solveLinearExpression(obj_func, x_k))
	...
	# libplot.simplePlot(x=plot_sol_diff, y=plot_obj_values, fig_title="Objective value w.r.t Solution Difference", x_axis_label="Solution Difference", y_axis_label="Objective function values", legend_text="Objective value")
...
def main_function():
	...
	if (solution != None):
		if (int(computeSolutionDifference(solution, initial_values, weights)) < deviation):
			op_result = "FAILED"
			solution = None
	...
	print '\nInitial solution vector provided : '
	...
	if (solution != None):
		print 'Solution Deviation               : ', int(computeSolutionDifference(solution, initial_values, weights))

	if (op_result == "FAILED_I"):
		libio.pprint(libio.RED_TEXT, '\nDINKELBACH FAILED. Reached iteration limit.\n')
	elif (op_result == "FAILED"):
		libio.pprint(libio.RED_TEXT, '\nDINKELBACH FAILED. Could not find a solution better than the status quo within the min deviation percentage. Try relaxing the parameters.\n')
	else:
		if (solution == None):
